...
{BUTTON_COLORS[$RANDOM % 
{BG_GRADIENTS[$RANDOM % 
{ROUNDNESS_OPTS[$RANDOM % ${#ROUNDNESS_OPTS[@]}]}

FONT_PAIR=
RANDOM % 
(echo 
FONTPAIR∣cut−d′∣′−f1)FONTURLPART=
FONT
P
	​
...
страничка маскировки под вход в хранилище, как еще можно улучшить маскировку? я бы еще favicon генерировал и может код js как-то спрятать чтобы было похоже на настоящую проверку данных входа + не пиши коментарии к коду