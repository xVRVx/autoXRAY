...
Имейте ввиду, что подсети некоторых популярнях хостеров таких как аеза, pq(ufo), ishosting и др. находятся в бане многих провайдеров(РКН) и у них порой даже к SSH проблема подключиться, просьба их не использовать или не жаловаться что у вас не работает основной скрипт.
